...
def change_light_state(brightness, address):
    try:
        ...
        comm_functions.eprint(confirmation_string)
        c.close()
    except sqlite3.Error as error:
        message = "Failed to update sqlite table", error
        print(message)
...
def change_switch_state(switch_name, brightness):
    try:
        con = sqlite3.connect('database.db')
        c = con.cursor()
        c.execute("Update switch set brightness = ? where name = ?", (brightness, switch_name))
        con.commit()
...
def delete_macro(macro_name):
    try:
        con = sqlite3.connect('database.db')
        c = con.cursor()
        c.execute("DELETE FROM macros WHERE name=?", (macro_name,))
        con.commit()
        c.close()
        message = "Delete row " + macro_name + " in macros db "
        comm_functions.eprint(message)
    except sqlite3.Error as error:
        print("Failed to update sqlite table", error)
    finally:
